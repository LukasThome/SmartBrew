...
int i, eixo;
float leiturasEixo[300] = {}, mediaLeituras = 0, somaLeituras = 0, variacao = 0, valorAtual = 0, valorInicial = 0;  
...
/*void makePOST()
{
  ...
  {
    //erial.println("Não foi possível conectar ao servidor!n");
  }
  else
  {    
...
void loop() {
  ...
  //makePOST();      // envia os dados ao servidor  
   ...
   for(i = 0; i < 300;i++){         
        ...
        for(i = 0; i < 300; i++){ 
           somaLeituras =  somaLeituras + leiturasEixo[i];
          }
            mediaLeituras =   somaLeituras/300;
            mediaLeituras = mediaLeituras / 100;
            ...
            Serial.print(" | Temp = "); Serial.println(Tmp/100);
